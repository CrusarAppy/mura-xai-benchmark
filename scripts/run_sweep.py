...
def main():
    ...
    print(f"Sweep plan: {len(backbones)} backbones x {len(methods)} methods "
          f"x {len(folds)} folds x {len(seeds)} seeds = {total} rows")
    done = 0

    for seed in seeds:
        set_seed(seed)
        cfg["experiment"]["seed"] = seed
        index = build_mura_index(d["mura_root"], split="train", regions=d.get("regions", "all"))
        ...
        for fold in folds:
            ...
            print(f"\n[seed {seed} | fold {fold}] train={len(tr_df)}  val={len(va_df)}")
            ...
            for backbone in backbones:
                ...
                ckpt = Path(cfg["output"]["checkpoints_dir"]) / f"{backbone}_f{fold}_s{seed}.pt"
                ...
                if ckpt.exists() and not force_retrain:
                    ...
                    print(f"  [train] {backbone} (fold {fold}, seed {seed})")
                    train_model(
                        model,
                ...
                # ---- model-level metrics (computed once, shared by all explainers) ----
                ...
                for method in methods:
                    ...
                    print(f"  === [{done}/{total}] {backbone} x {method} ===")
